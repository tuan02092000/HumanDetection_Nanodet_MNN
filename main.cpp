...
int image_demo(NanoDet &detector, const char* imagepath)
{
    ...
    for (auto img_name : filenames)
    {
        ...
        // resize_uniform(image, resized_img, cv::Size(160, 160), effect_roi);
        // resize_uniform(image, resized_img, cv::Size(128, 128), effect_roi);
...
int benchmark(NanoDet& detector)
{
    ...
    // cv::Mat image(160, 160, CV_8UC3, cv::Scalar(1, 1, 1));
    cv::Mat image(128, 128, CV_8UC3, cv::Scalar(1, 1, 1));
...
int main(int argc, char** argv)
{
    ...
    // NanoDet detector = NanoDet("../model/nanodet-128_3_11.mnn", 128, 128, 4, 0.45, 0.3);
